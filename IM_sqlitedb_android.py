	 	...
	 	art.addAttribute(BlackboardAttribute(BlackboardAttribute.ATTRIBUTE_TYPE.TSK_MESSAGE_TYPE, 
            ...
            try:
                if resultSet is not None:
                    resultSet.close()                    
                if resultSet_contacts is not None:
            ...
            if vk_files.index(file) == 0:
                message = IngestMessage.createMessage(IngestMessage.MessageType.DATA,
                                                      "IM SQliteDB Analyzer", "Обнаружены базы данных %s ".decode('UTF-8') % "VK (ВКонтакте)".decode('UTF-8'))
	...
	for file in kate_files:		
	 	...
	 	art.addAttribute(BlackboardAttribute(BlackboardAttribute.ATTRIBUTE_TYPE.TSK_MESSAGE_TYPE, 
            ...
            if kate_files.index(file) == 0:
                message = IngestMessage.createMessage(IngestMessage.MessageType.DATA,
                                                      "IM SQliteDB Analyzer", "Обнаружены базы данных %s ".decode('UTF-8') % "Kate Mobile (ВКонтакте)".decode('UTF-8'))
                IngestServices.getInstance().postMessage(message)
#End vk
...
#Extract viber
	for file in viber_calls_files:		
            ...
            if viber_calls_files.index(file) == 0:
                message = IngestMessage.createMessage(IngestMessage.MessageType.DATA,
                                                      "IM SQliteDB Analyzer", "Обнаружены базы данных %s ".decode('UTF-8') % "Viber (звонки)".decode("UTF-8"))
                IngestServices.getInstance().postMessage(message)
	...
	for file in viber_messages_files:		
            ...
            if viber_messages_files.index(file) == 0:
                message = IngestMessage.createMessage(IngestMessage.MessageType.DATA,
                                                      "IM SQliteDB Analyzer", "Обнаружены базы данных %s ".decode('UTF-8') % "Viber (сообщения)".decode("UTF-8"))
                IngestServices.getInstance().postMessage(message)                
#End viber
...
#Extract skype
	for file in skype_files:		
            ...
            try:                
                ...
                resultSet_contacts = stmt3.executeQuery("select Contacts.[fullname], Contacts.[skypename],  Contacts.[birthday], Contacts.[phone_home], Contacts.[phone_office],  Contacts.[phone_mobile], Contacts.[emails], Contacts.[homepage], Contacts.[about] from Contacts".decode('UTF-8'))
                resultSet_calls = stmt4.executeQuery("select Calls.[id], CallMembers.[type], Calls.[host_identity], CallMembers.[identity],  CallMembers.[dispname] , Calls.[begin_timestamp], Calls.[duration],  CallMembers.[guid] from Calls, CallMembers where Calls.[id]=CallMembers.[call_db_id]".decode('UTF-8'))                
            except SQLException as e:
            ...
            try:
                if resultSet_messages is not None:
                    resultSet_messages.close()
                if account_info is not None:
                    account_info.close()
                if resultSet_contacts is not None:
                    resultSet_contacts.close()
                if resultSet_calls is not None:
            ...
            if skype_files.index(file) == 0:
                message = IngestMessage.createMessage(IngestMessage.MessageType.DATA,
                                                      "IM SQliteDB Analyzer", "Обнаружены базы данных %s ".decode('UTF-8') % "Skype".decode('UTF-8'))
                IngestServices.getInstance().postMessage(message)
#End Skype
...
#Extract gmail messages
	for file in gmail_files:		
            ...
            if gmail_files.index(file) == 0:
                message = IngestMessage.createMessage(IngestMessage.MessageType.DATA,
                                                      "IM SQliteDB Analyzer", "Обнаружены базы данных %s ".decode('UTF-8') % "Gmail".decode('UTF-8'))
                IngestServices.getInstance().postMessage(message)
#End gmail
...
#Extract AquaMail messages
	for file in aquamail_files:		
            ...
            if aquamail_files.index(file) == 0:
                message = IngestMessage.createMessage(IngestMessage.MessageType.DATA,
                                                      "IM SQliteDB Analyzer", "Обнаружены базы данных %s ".decode('UTF-8') % "Aquamail".decode('UTF-8'))
                IngestServices.getInstance().postMessage(message)
#End Aquamail               
...
#Extract odnoklassniki
	for file in odnoklassniki_files:		
            ...
            try:
                ...
                while columnListResultSet.next():
                    if columnListResultSet.getString("COLUMN_NAME") == "message":
                        ...
                        break
                if columnFound:
                    resultSet_messages = stmt3.executeQuery("select messages.[_id], messages.[author_id], (select users.[name] from users where messages.author_id=users._id) as user_name, messages.[message], messages.[_date] from messages order by messages._date".decode('UTF-8'))
                else:
                    try:
            ...
            try:
                if resultSet_account_info is not None:
            ...
            if odnoklassniki_files.index(file) == 0:
                message = IngestMessage.createMessage(IngestMessage.MessageType.DATA,
                                                      "IM SQliteDB Analyzer", "Обнаружены базы данных %s ".decode('UTF-8') % "\"Одноклассников\"".decode('UTF-8'))
                IngestServices.getInstance().postMessage(message)
	...
	for file in odnoklassniki_files_tam:		
		...
		art.addAttribute(BlackboardAttribute(attID_companion, IMDbIngestModuleFactory.moduleName, user_name))
            ...
            if odnoklassniki_files_tam.index(file) == 0:
                message = IngestMessage.createMessage(IngestMessage.MessageType.DATA,
                                                      "IM SQliteDB Analyzer", "Обнаружены базы данных %s ".decode('UTF-8') % "\"Одноклассников\" (tamtam_messages)".decode('UTF-8'))
                IngestServices.getInstance().postMessage(message)                
#End odnoklassniki
