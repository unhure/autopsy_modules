...
def aquamail(self, progressBar, aquamail_files):
    ...
    try:
        artID_aquamail = case.getSleuthkitCase().addArtifactType(
            "TSK_CHATS_AQUAMAIL", "Aquamail - электронная почта".decode("UTF-8")
        )
    except:
    ...
    try:
        attID_sender = case.getSleuthkitCase().addArtifactAttributeType(
            "TSK_MESS_OTPRAV",
            blackboardAttribute.TSK_BLACKBOARD_ATTRIBUTE_VALUE_TYPE.STRING,
            "Пользователь, отправивший сообщение".decode("UTF-8"),
        )
    except:
        attID_sender = case.getSleuthkitCase().getAttributeType("TSK_MESS_OTPRAV")
    try:
        attID_reciever = case.getSleuthkitCase().addArtifactAttributeType(
            "TSK_MESS_RECIEVER",
            blackboardAttribute.TSK_BLACKBOARD_ATTRIBUTE_VALUE_TYPE.STRING,
            "Пользователь, получивший сообщение".decode("UTF-8"),
        )
    except:
    ...
    for file in aquamail_files:
        ...
        try:
            ...
            self.log(
                Level.INFO,
                "Error querying database for aquamail (" + e.getMessage() + ")",
            )
        ...
        if "resultSet_messages" in locals():
            while resultSet_messages.next():
                try:
                    ...
                    preview_attachments = resultSet_messages.getString(
                        "preview_attachments"
                    )
                    ...
                    self.log(
                        Level.INFO,
                        "Error getting values from messages table (aquamail) ("
                        + e.getMessage()
                        + ")",
                    )
                ...
                art.addAttribute(
                    blackboardAttribute(
                        blackboardAttribute.ATTRIBUTE_TYPE.TSK_DATETIME_SENT.getTypeID(),
                ...
                art.addAttribute(
                    blackboardAttribute(
                        attID_sender, imdbIngestModuleFactory.moduleName, author
                    )
                )
                art.addAttribute(
                    blackboardAttribute(
                ...
                art.addAttribute(
                    blackboardAttribute(
                        attID_status, imdbIngestModuleFactory.moduleName, status
                    )
                )
        ...
        if aquamail_files.index(file) == 0:
            message = IM_sqlitedb_android.IngestMessage.createMessage(
                ...
                imdbIngestModuleFactory.moduleName,
                "Обнаружены базы данных: Aquamail".decode("UTF-8"),
            )
